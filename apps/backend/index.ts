...
let requestId : {request_id : string }[] = await Promise.all(prompts.map(async (prompt) => falAiModel.generateImages(prompt.prompt , parsedData.data.modelId)))
...
res.json({
    imageId : imageData.map((image) => image.id)
})
})
